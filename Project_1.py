    ...
    def quick_sort(self):  # quick sort function more like how i did it before but now in line
        def swapper(low, high):       # the window of our array
            ...
            for j in range(low, high):          # for all elements in our window
                ...
                # ^^ then we swap it with the next window so we know that elements less then the window are really small
        ...
        def quickSort(low, high):              # this is where we have the recursive calls
            ...
            if low < high:
                piv = swapper(low, high)          # this is the inilization of the swapper
                quickSort(low, piv - 1)           # then this will call itself for the lower array
                quickSort(piv + 1, high)          # this will call itself for the upper array
